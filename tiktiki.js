// ok here we go again. js. my beloved. my enemy.
"use strict";
...
// neoxy char ramps. Each entry [char, brightness] — brightness = measured pixel
// coverage of the glyph, so darker cells pick sparser characters.
const base = [
    ["~", 12198],
    [":", 6921],
    ["-", 5589],
...
const speed_1 = 1.0 / 60.0 / 16;     // while dragging
const speed_base = 1.0 / 60.0 / 3;
const speed_2 = 1.0 / 60.0 / 1.25;   // released

const real_width =
    Math.ceil(window.innerWidth / grid_size + cell_crop_x * 2) * grid_size;
const real_height =
    Math.ceil(window.innerHeight / grid_size + cell_crop_y * 2) * grid_size;
...
// Monospace glyphs are taller than they are wide, so a row of N chars would only
// fill ~60% of the width if we did nothing. Measure the actual glyph advance at our
// font size and add letter-spacing so each character occupies exactly grid_size px.
// That makes the on-screen grid truly square, which the click math depends on.
...
// SIX HUNDREAD LINES OF CODE
// AND ITS STILL mostly ONE CLASS
// why did i do this (again)
// ----------------- start of simulator ------------------------------
class FlipFluid {
    constructor(density, width, height, spacing, particle_radius, max_particles) {
        ...
        this.cell_type = new Int32Array(this.f_num_cells);
        ...
        this.p_inv_spacing = 1.0 / (2.2 * particle_radius);
        ...
        this.first_cell_particle = new Int32Array(this.p_num_cells + 1);
        this.cell_particle_ids = new Int32Array(max_particles);
    ...
    // kick every particle outward from the tank center with a random burst -> explosion
    // the funny button function. press s and watch it all fall apart (literally)
    shake(strength) {
        ...
        for (var i = 0; i < this.num_particles; i++) {
            ...
            var d = Math.sqrt(dx * dx + dy * dy) || 1.0;
            // outward direction + a healthy dose of randomness so it looks chaotic
            var ox = dx / d, oy = dy / d;
    ...
    // spawn a small clump of new fluid at (sx, sy) in sim space -> click to form
    // makes more water out of nothing. very godlike. unused rn but i kept it bc effort
    spawn_at(sx, sy, count) {
        var r = this.particle_radius;
        for (var k = 0; k < count; k++) {
            if (this.num_particles >= this.max_particles) break;
            var i = this.num_particles++;
            // scatter in a little disc around the cursor
    ...
    // shove the little guys apart so they dont clip into each other
    // this is the part that was eating my soul at 2am
    push_particles_apart(num_iters) {
        ...
        for (var i = 0; i < this.num_particles; i++) {
            ...
            this.num_cell_particles[xi * this.p_num_y + yi]++;
        }
        var first = 0;
        for (var i = 0; i < this.p_num_cells; i++) {
            first += this.num_cell_particles[i];
            this.first_cell_particle[i] = first;
        }
        this.first_cell_particle[this.p_num_cells] = first;
        ...
        for (var i = 0; i < this.num_particles; i++) {
            ...
            this.first_cell_particle[cell_nr]--;
            ...
            for (var i = 0; i < this.num_particles; i++) {
                ...
                for (var xi = x0; xi <= x1; xi++) {
                    for (var yi = y0; yi <= y1; yi++) {
                        var cell_nr = xi * this.p_num_y + yi;
                        var first_c = this.first_cell_particle[cell_nr];
                        var last_c = this.first_cell_particle[cell_nr + 1];
    ...
    // 6 sides. six. why not a circle. circles are ONE line of math. anyway
    // ---- hexagon collision (flat-top) ----
    handle_particle_collisions(obstacle_x, obstacle_y, obstacle_radius) {
        ...
        var min_y = h + r, max_y = (this.f_num_y - 1) * h - r;

        // Build flat-top hexagon vertices once. Flat-top => first edge is horizontal,
        // so we start the angle at 0 and step by 60°.
        var R = obstacle_radius;
        var hex = [];
        for (var k = 0; k < 6; k++) {
            var ang = (Math.PI / 3) * k;                // 0,60,120,...
        ...
        // convex polygon point test: inside iff all cross products share a sign
        ...
        for (var i = 0; i < this.num_particles; i++) {
            ...
            if (R > 0 && in_hex(x, y)) {
                // find nearest point on the hex boundary and snap the particle to it
    ...
    // counts how squished the water is. for the drift compensation thingy
    update_particle_density() {
        ...
        if (this.particle_rest_density == 0.0) {
            ...
            for (var i = 0; i < this.f_num_cells; i++) {
                if (this.cell_type[i] == fluid_cell) { sum += d[i]; num_fluid_cells++; }
            }
            if (num_fluid_cells > 0) this.particle_rest_density = sum / num_fluid_cells;
        }
    }

    // grid <-> particles. the actual FLIP in flip fluid. do NOT touch this i will cry
    // i do not fully understand it i just know it works (-_-)
    transfer_velocities(to_grid, flip_ratio) {
        ...
        for (var component = 0; component < 2; component++) {
            ...
            if (to_grid) {
                for (var i = 0; i < f.length; i++) if (d_arr[i] > 0.0) f[i] /= d_arr[i];
    ...
    // the math heavy one. makes water act like water and not like fog
        ...
        for (var iter = 0; iter < num_iters; iter++) {
            for (var i = 1; i < this.f_num_x - 1; i++) {
                for (var j = 1; j < this.f_num_y - 1; j++) {
                    ...
                    var center = i * n + j, left = (i - 1) * n + j, right = (i + 1) * n + j, bottom = i * n + j - 1, top = i * n + j + 1;
                    var sx0 = this.s[left], sx1 = this.s[right], sy0 = this.s[bottom], sy1 = this.s[top];
    ...
    // turns a number into a brightness. used to be rainbow. now its just grey :(
    ...
    // ok this is the one that runs everything once per frame. the conductor
    // look at all these arguments. so many arguments. i am so sorry
    simulate(dt, flip_ratio, num_pressure_iters, num_particle_iters, over_relaxation, compensate_drift, separate_particles, obstacle_x, obstacle_y, obstacle_radius) {
        this.integrate_particles(dt);
        if (separate_particles) this.push_particles_apart(num_particle_iters);
...
// the big settings bag. change these numbers and pray
var scene = {
    dt: speed_base,
    flip_ratio: 0.9,
    ...
    obstacle_x: 0.0,
    obstacle_y: 0.0,
    target_x: 0.0,        // where the cursor wants the hexagon; it eases toward this
    target_y: 0.0,
    obstacle_radius: 0,
    target_radius: 0.18,   // sim-space target, driven by the slider
    follow_speed: 0.18,    // 0..1 per frame; lower = slower, gentler displacement
    paused: false,
    fluid: null,
};
...
// builds the tank + dumps a bunch of water in. the big bang basically
function setup_scene() {
    var res = resolution;
    var tank_height = 1.0 * sim_height;
    var tank_width = 1.0 * sim_width;
    var h = tank_height / res;
    var density = 1000.0;
    ...
    // headroom so click-to-form can add more fluid
...
// set where the hexagon should head. It eases toward this in update(), so the
// fluid gets pushed gradually instead of a violent one-frame teleport.
// moves the hexagon. just sets x/y, the collision code does the real work
function set_obstacle(x, y, reset) {
    scene.target_x = x;
    scene.target_y = y;
    if (reset) {
        // on a fresh press, snap the *target* but let the body glide there
        scene.target_x = x;
        scene.target_y = y;
    }
}
...
// screen pixels -> sim coords. this one BROKE EVERYTHING for like 3 messages straight
...
function end_drag() { mouse_down = false; }

render_el.addEventListener("mousedown", (e) => { scene.dt = speed_1; start_drag(e.clientX, e.clientY); });
window.addEventListener("mouseup", () => { scene.dt = speed_2; end_drag(); });
window.addEventListener("mousemove", (e) => drag(e.clientX, e.clientY));

render_el.addEventListener("touchstart", (e) => { e.preventDefault(); scene.dt = speed_1; start_drag(e.touches[0].clientX, e.touches[0].clientY); }, { passive: false });
window.addEventListener("touchend", () => { scene.dt = speed_2; end_drag(); });
render_el.addEventListener("touchmove", (e) => { e.preventDefault(); drag(e.touches[0].clientX, e.touches[0].clientY); }, { passive: false });

// slider: map px size -> sim-space radius, live
function apply_slider_size() {
    var px = parseInt(size_slider.value, 10);
    size_val.textContent = px;
    // px is roughly the hex radius in screen pixels; convert to sim units
...
document.addEventListener("keydown", (e) => {
    ...
    if (e.key === "s") { scene.paused = false; f.shake(6.0); }
});
...
function setup_device_motion() {
    window.addEventListener("devicemotion", (event) => {
        ...
        if (!x && !y) return;
        if (window.orientation === 90 || window.orientation === -90) { const t = x; x = -y; y = t; }
        window.gravity_vector = { x, y };
    });
}

// main -------------------------------------------------------
// (the global one, not the class one. yes there are two. dont @ me)
function simulate() {
    if (!scene.paused) {
        scene.fluid.simulate(
            scene.dt, scene.flip_ratio, scene.num_pressure_iters, scene.num_particle_iters,
            scene.over_relaxation, scene.compensate_drift, scene.separate_particles,
...
// the render loop. runs forever. turns physics into letters. magic
// if youre reading this hi. its 1am. send help
function update() {
    // ease the live radius toward the slider target so growth/shrink looks smooth
    scene.obstacle_radius = scene.obstacle_radius * 0.8 + scene.target_radius * 0.2;

    // glide the hexagon toward the cursor target instead of snapping, so it
    // displaces fluid gradually rather than exploding it on a click
...
setup_scene();
// drop the hexagon in the middle of the *visible* (cropped) grid.
// IMPORTANT: set BOTH the position and the easing target, otherwise update()
// glides it toward target (0,0) = bottom-left on the first frames. that bug
// haunted me. never again.
